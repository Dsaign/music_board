...
function createVisual(analyser) {
  ...
  let animationTime = 0;
  const animationDuration = 3800; // Duração total da animação em ms
  ...
  const draw = (currentTime) => {
    ...
    if (isPlaying && audioLoaded) {
      ...
      // Modo de animação de onda com ease-in-out
      ...
      wavePosition = easeOutCubic(normalizedProgress) * (arcs.length + totalWaveWidth * 2)  - totalWaveWidth;
      ...
      arcs.forEach((arc, index) => {
        ...
        const lineWidth = 1 + (maxLineWidth * waveIntensity);
        const opacity = 0.3 + (0.2 * waveIntensity);
        
        // Se o arco está dentro da largura da onda
        // if (distanceFromWave < totalWaveWidth) {
        //   // Calcular a intensidade baseada na posição dentro da onda
        //   // Mais intenso no centro da onda, menos nas bordas
        //   const waveIntensity = 1 - (distanceFromWave / totalWaveWidth);
        //   lineWidth = 1 + (maxLineWidth * waveIntensity) * 0.7;
        //   opacity = 0.3 + (0.1 * waveIntensity);
        // }
